...
Convert <- function(abt, feats, ref)
{
  ...
  stopifnot(colnames(ref) == c("name", "value"), 
            colnames(feats) == c("name", "value"))
  
  idx <- numeric()
  ref <- paste(ref$name, ref$value, sep = ":")
  feats <- paste(feats$name, feats$value, sep = ":")
  for (i in ref) {
    idx <- append(idx, if (i %in% feats) which(feats == i) else 0)
  }
  idx
  
  blank <- rep(0, nrow(abt))
  
  out <- do.call(cbind, lapply(idx, function(x) {
    if (x == 0) blank else abt[, x]}))
  colnames(out) <- ref
  return(out)
}
