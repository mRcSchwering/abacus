#' Read_csv
#'
#' This function is used to read a \emph{csv} file and create a \emph{Transactions} object from it.
#' This object contains the transactions in a standardized form and information about the reference account.
#' 
#' \enumerate{
...
#'  \item table is \emph{converted} as specified with arguments \code{columns} and \code{dateFormat}
#'  \item \emph{Transactions} object is returned
#' }
#'
#' @family procedures
#'
#' @param ref          \code{chr} for the \emph{type} of the reference account of the uploaded file
#' @param fileName     \code{chr} for the file to be read (should be a \emph{csv})
#' @param columns      \code{list} which defines which columns of the read table (from file) contain which information:
#'                     It must specify \emph{name}, \emph{iban}, \emph{bic}, \emph{date}, 
#'                     \emph{reference}, \emph{entry}, \emph{value}, \emph{currency} as list elements in this order,
#'                     with their according columns in the table as \code{int} values.
#' @param db           \code{chr} the database used / file name and path of database
#' @param head         \code{bool} (=\code{TRUE}) whether the first line of the file defines column names of the table
#' @param colSep       \code{chr} (=\code{"\\t"}) seperator for columns in the table
#' @param decSep       \code{chr} (=\code{","}) decimal seperator for numbers
#' @param quoteChar    \code{chr} (=\code{"\\""}) symbol used for quotes in table
#' @param commentChar  \code{chr} (=\code{""}) symbol introducing comments in the table
#' @param nSkip        \code{int} (=\code{0}) how many lines to skip when reading the file (from top to bottom)
#' @param nMax         \code{int} (=\code{-1}) maximum number of lines the read (negative to read all lines of file)
#' @param dateFormat   \code{chr} (=\code{"\%d.\%m.\%Y"} for"13.03.1990") date format to use for converting the date column
#' 
                      ...
                      quoteChar = "\"", commentChar = "", nSkip = 0, nMax = -1, dateFormat = "%d.%m.%Y" )
{
  ...
  df <- as.data.frame(do.call(cbind, lapply(columns, function(x) csv[, x])), stringsAsFactors = FALSE)
