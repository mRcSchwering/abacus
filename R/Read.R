...
#' \code{\link{Read.Transactions}} is used to read a \emph{Transactions} 
#' object (created with \code{\link{Read_csv}} for example).
#' 
#' @family procedures
#' 
#' @export
...
#' Read
#'
#' This procedure identifies accounts occuring in a \emph{Transactions} object 
#' which cannot be found in the associated database.
#' It then adds these accounts to the \emph{Transactions} object and returns it.
#'
#' @family procedures
#'
#' @param x          \code{Transactions} object 
#'                   (created with \code{\link{Read_csv}} for example)
#' 
...
#'     \item \emph{NewAccounts} a data.frame of accounts that appear in 
#'     \emph{Transactions}, but not in the database
#' }
#' 
#' @examples 
#' db <- "test.db"
...
{
  tas <- x$Transactions
  acc <- tas[!duplicated(tas[, c("iban", "bic")]), 
             c("name", "iban", "bic")]
  acc <- acc[!Intersect(acc, "accounts", x$db), 
             c("name", "iban", "bic")]
  names(acc)[1] <- "owner"
  x[["NewAccounts"]] <- acc
  return(x)
