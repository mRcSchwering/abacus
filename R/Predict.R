...
#' \code{\link{Predict.Transactions}} is used to predict transaction types 
#' of a  \emph{Transactions} object (created with 
#' \code{\link{Read.Transactions}} for example).
#' 
#' @family procedures
#' 
#' @export
...
#' transaction for a \emph{Transactions} object
...
#' 
#' \enumerate{
#'     \item If there are \emph{new accounts} (not existent in database yet), 
#'     these accounts are written into the database.
#'     Also, if the user did make changes to these accounts, 
#'     these changes are applied to the actual transactions accordingly.
#'     \item A complete \emph{transactions} table is created by combining the 
#'     original transactions table with all
#'     information about the accounts. Numeric values are converted to Integers 
#'     in this step (* 100).
#'     \item If a \emph{Prediction Model} was already created, it is read 
#'     from the database
#'     \item \emph{Feature Extraction} and \emph{Prediction} are done using 
#'     the model
#'     
#' }
#'
...
#'                   (created with \code{\link{Read.Transactions}} for example)
#' 
#' @return 
...
#'     \item \emph{Prediction} a data.frame of transactions with additional 
#'     columns of account information.
...
{
  if (!"NewAccounts" %in% names(x)) {
    stop("Use Read method with Transactions object first")
  }
  if (any(nchar(x$NewAccounts$owner) < 1)) {
    stop("At least 1 owner has no name")
  }
  if (any(grepl("[^0-9A-Za-z ]", x$NewAccounts$owner))) {
    stop("Please only use letters and numbers as owner names")
  }
  if (any(is.na(x$Transactions$value))) {
    stop("NA's in transaction values")
  }
  ...
  if (nrow(x$NewAccounts) > 0) {
    ...
    for (i in 1:nrow(x$NewAccounts)) {
      idx <- cbind(x$NewAccounts$iban[i] == x$Transactions$iban, 
                   x$NewAccounts$bic[i] == x$Transactions$bic)
  ...
  l <- lapply(1:nrow(x$Transactions), function(i) {
    r <- x$Transactions[i, ]
    sgn <- r$value > 0
    acc <- Select("accounts", x$db, 
                  eq = list(iban = r$iban, bic = r$bic), all_and = TRUE)
    data.frame(
